...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for pair in parallel_corpus:
            source_tokens = pair.source_tokens 
            ...
            for i in range(m):
                t = target_tokens[i]
                probs = self.translation_probs[source_tokens, t]
                total = np.sum(probs)
                if total == 0:
                    posterior[:, i] = 1 / n
                else:
                    posterior[:, i] = probs / total
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for pair, posterior in zip(parallel_corpus, posteriors):
            src_tokens = pair.source_tokens
            tgt_tokens = pair.target_tokens

            p_mat = self.translation_probs[src_tokens, :][:, tgt_tokens]

            term = posterior * (np.log((1 / len(src_tokens)) * p_mat) - np.log(posterior))
            elbo += np.sum(term)
        return elbo
        ...
        for pair, posterior in zip(parallel_corpus, posteriors):
            ...
            for j in range(n):
                for i in range(m):
                    x = src_tokens[j]
                    y = tgt_tokens[i]
                    counts[x, y] += posterior[j, i]
        for x in range(self.num_source_words):
            total = np.sum(counts[x, :])
            if total > 0:
                self.translation_probs[x, :] = counts[x, :] / total
    ...
    def align(self, sentences):
        src_tokens = sentences.source_tokens
        tgt_tokens = sentences.target_tokens
        n = len(src_tokens)
        m = len(tgt_tokens)
        posterior = np.zeros((n, m), dtype=np.float32)
        for i in range(m):
            t = tgt_tokens[i]
            probs = self.translation_probs[src_tokens, t]
            total = np.sum(probs)
            if total == 0:
                posterior[:, i] = 1 / n
            else:
