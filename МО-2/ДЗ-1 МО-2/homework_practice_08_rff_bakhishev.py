...
class RandomFeatureCreator(FeatureCreatorPlaceholder):
    def fit(self, X, y=None):
        choice_num = 10 ** 6
        sample_1 = np.random.choice(X.shape[0], choice_num)
        sample_2 = np.random.choice(X.shape[0], choice_num)
        sigma = np.sqrt(np.median(((X[sample_1] - X[sample_2]) ** 2).sum(axis=1)))
        self.w = 1 / sigma * np.random.normal(0, 1, (self.n_features, self.new_dim))
        self.b = np.random.uniform(-np.pi, np.pi, (self.n_features, 1))
        return self

    def transform(self, X, y=None):
        return self.func(self.w @ X.T + self.b).T
...
class OrthogonalRandomFeatureCreator(RandomFeatureCreator):
    def fit(self, X, y=None):
        ...
        copies = int(np.ceil(D / d))
        blocks = []
        for _ in range(copies):
            G = np.random.normal(0, 1, (d, d))
            Q, R = np.linalg.qr(G)
            diag = np.sqrt(np.random.chisquare(d, d))
            S = np.diag(diag)
            blocks.append(1 / sigma * (S @ Q))

        self.w = np.vstack(blocks)[:D, :]
        self.b = np.random.uniform(-np.pi, np.pi, (D, 1))
        return self
